...
create table if not exists public.formalizacoes_postos (
    ...
    tipo text not null,
    status text not null default 'formalizado',
    ...
    email_body text,
    whatsapp_text text,
    constraint formalizacoes_postos_tipo_check check (
        tipo in (
            'troca_posto',
            'remanejamento',
            'desligamento',
    ...
    constraint formalizacoes_postos_status_check check (
        status in ('formalizado', 'em_analise', 'concluido', 'cancelado')
    ),
    constraint formalizacoes_postos_prioridade_check check (
...
create table if not exists public.formalizacoes_status_events (
    ...
    observacao text,
    created_at timestamptz not null default now(),
    constraint formalizacoes_status_events_status_anterior_check check (
        status_anterior is null
        or status_anterior in ('formalizado', 'em_analise', 'concluido', 'cancelado')
    ),
    constraint formalizacoes_status_events_status_novo_check check (
        status_novo in ('formalizado', 'em_analise', 'concluido', 'cancelado')
    )
);

create index if not exists idx_formalizacoes_postos_created_at
    on public.formalizacoes_postos (created_at desc);

create index if not exists idx_formalizacoes_postos_protocolo
    on public.formalizacoes_postos (protocolo);

create index if not exists idx_formalizacoes_postos_status
    on public.formalizacoes_postos (status);

create index if not exists idx_formalizacoes_postos_tipo
    on public.formalizacoes_postos (tipo);

create index if not exists idx_formalizacoes_postos_colaborador
    on public.formalizacoes_postos (colaborador_matricula, colaborador_re, colaborador_nome);

create index if not exists idx_formalizacoes_postos_postos
    on public.formalizacoes_postos (posto_atual, posto_destino);

create index if not exists idx_formalizacoes_status_events_formalizacao
    on public.formalizacoes_status_events (formalizacao_id, created_at);
...
for insert with check (
    ...
    and status in ('formalizado', 'em_analise', 'concluido', 'cancelado')
);
    ...
    if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
        if not exists (
            ...
            where pubname = 'supabase_realtime'
              ...
              and tablename = 'formalizacoes_postos'
        ) then
        ...
        if not exists (
            ...
            where pubname = 'supabase_realtime'
              and schemaname = 'public'
              and tablename = 'formalizacoes_status_events'
        ) then
