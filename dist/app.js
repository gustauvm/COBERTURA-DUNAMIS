        ...
        <div class="result-card ${x} ${h?"card-homenageado":""} ${w}" data-collab-re="${escapeHtml(e.re||"")}" data-collab-id="${e.id}" data-preview="${G}" style="border-left: 5px solid ${o.color}">
            <div class="card-header">
                ...
                <div class="header-right card-actions-bar">
                    ...
                    <button class="edit-btn-icon map-icon ${R}" onclick="openAddressModalForCollaborator(${f}, ${u}, ${v})" title="${D}" ${_?"":"disabled"}>${ICONS.mapPin}</button>
...
`))}function getFormalizadorDefaultDraft(){return{step:1,type:"",requester:{nome:"",cargo:"",telefone:"",email:""},collaboratorKey:"",destinationKey:"",coverageKey:"",queries:{collaborator:"",destination:"",coverage:""},form:{prioridade:"normal",data_efetiva:getTodayKey(),data_fim:"",motivo_categoria:"",motivo_observacao:"",email_recipients:""},benefits:{vale_transporte:!1,vale_refeicao:!1,adicional_noturno:!1,intrajornada:!1,escala_turno:!1,observacoes:""},coverage:{tipo:"sem_cobertura",periodo:"",observacoes:""}}}function mergeFormalizadorDraft(e={}){formalizadorState={...formalizadorState,...e,requester:{...formalizadorState.requester,...e.requester||{}},queries:{...formalizadorState.queries,...e.queries||{}},form:{...formalizadorState.form,...e.form||{}},benefits:{...formalizadorState.benefits,...e.benefits||{}},coverage:{...formalizadorState.coverage,...e.coverage||{}}}}function loadFormalizadorRequester(){try{const e=localStorage.getItem(FORMALIZADOR_REQUESTER_KEY);if(!e)return;const t=JSON.parse(e);if(!t||typeof t!="object")return;formalizadorState.requester={nome:String(t.nome||""),cargo:String(t.cargo||""),telefone:String(t.telefone||""),email:String(t.email||"")}}catch{}}function saveFormalizadorRequester(){try{localStorage.setItem(FORMALIZADOR_REQUESTER_KEY,JSON.stringify(formalizadorState.requester||{}))}catch{}}function resetFormalizadorDraft(){const e={...formalizadorState.requester||{}},t=getFormalizadorDefaultDraft();formalizadorState={...formalizadorState,...t,requester:e,history:formalizadorState.history||[],events:formalizadorState.events||{},historyFilters:formalizadorState.historyFilters||{search:"",status:"all",tipo:"all"},selectedHistoryId:formalizadorState.selectedHistoryId||"",lastCreatedId:formalizadorState.lastCreatedId||""},renderFormalizador()}async function activateFormalizador(){const e=document.getElementById("formalizador-root");e&&(e.innerHTML='<div class="formalizador-loading">Carregando colaboradores, unidades e hist\xF3rico...</div>'),loadFormalizadorRequester(),formalizadorState.form?.data_efetiva||mergeFormalizadorDraft({form:{data_efetiva:getTodayKey()}});try{await Promise.all([fetchSupabaseCollaborators(!1),fetchSupabaseUnits(!1),fetchFormalizacoes(!1)])}catch(t){AppErrorHandler.capture(t,{scope:"formalizador-activate"},{silent:!0})}renderFormalizador()}async function fetchFormalizacoes(e=!1){if(!supabaseClient)return formalizadorState.history||[];const t=Date.now();if(!e&&formalizadorCache.items&&t-formalizadorCache.updatedAt<SUPABASE_CACHE_TTL_MS)return formalizadorState.history=formalizadorCache.items||[],formalizadorState.events=formalizadorEventsCache.items||formalizadorEventsCache||{},formalizadorState.history;try{const{data:a,error:o}=await supabaseClient.from(SUPABASE_TABLES.formalizacoes_postos).select("*").order("created_at",{ascending:!1}).limit(500);if(o)throw o;return formalizadorCache={items:a||[],updatedAt:t,error:""},formalizadorState.history=formalizadorCache.items,await fetchFormalizacaoEvents(e),formalizadorState.history}catch(a){return formalizadorCache={...formalizadorCache||{},error:String(a?.message||a||"Erro ao carregar formaliza\xE7\xF5es")},AppErrorHandler.capture(a,{scope:"formalizador-fetch-history"},{silent:!0}),formalizadorState.history||[]}}async function fetchFormalizacaoEvents(e=!1){if(!supabaseClient)return formalizadorState.events||{};const t=Date.now();if(!e&&formalizadorEventsCache.updatedAt&&t-formalizadorEventsCache.updatedAt<SUPABASE_CACHE_TTL_MS)return formalizadorState.events=formalizadorEventsCache.items||{},formalizadorState.events;try{const{data:a,error:o}=await supabaseClient.from(SUPABASE_TABLES.formalizacoes_status_events).select("*").order("created_at",{ascending:!1}).limit(1200);if(o)throw o;const n={};return(a||[]).forEach(s=>{const i=String(s.formalizacao_id||"");i&&(n[i]||(n[i]=[]),n[i].push(s))}),Object.keys(n).forEach(s=>{n[s].sort((i,r)=>Date.parse(i.created_at||"")-Date.parse(r.created_at||""))}),formalizadorEventsCache={items:n,updatedAt:t,error:""},formalizadorState.events=n,n}catch(a){return formalizadorEventsCache={...formalizadorEventsCache||{},error:String(a?.message||a||"Erro ao carregar eventos")},AppErrorHandler.capture(a,{scope:"formalizador-fetch-events"},{silent:!0}),formalizadorState.events||{}}}function getFormalizadorCollaborators(){return((allCollaboratorsCache.items&&allCollaboratorsCache.items.length?allCollaboratorsCache.items:currentData||[])||[]).map((t,a)=>({key:getFormalizadorCollaboratorKey(t,a),item:t,unit:getFormalizadorUnitForCollaborator(t)}))}function getFormalizadorUnits(){return(supaUnitsCache.items||[]).map((e,t)=>({key:getFormalizadorUnitKey(e,t),unit:e}))}function getFormalizadorCollaboratorKey(e,t=0){return String(e?.matricula||e?.re||e?.re_padrao||e?.re_novo||`${e?.nome||e?.colaborador||"colaborador"}-${t}`).trim()}function getFormalizadorUnitKey(e,t=0){return normalizeUnitKey(e?.posto||e?.nome||e?.cliente||`unidade-${t}`)}function getFormalizadorUnitForCollaborator(e){const t=supaUnitsCache.items||[];return t.length&&buildQuickBetaUnitIndex(t).get(normalizeUnitKey(e?.posto||""))||null}function getSelectedFormalizadorCollaborator(){const e=String(formalizadorState.collaboratorKey||"");return getFormalizadorCollaborators().find(t=>t.key===e)||null}function getSelectedFormalizadorDestination(){const e=String(formalizadorState.destinationKey||"");return getFormalizadorUnits().find(t=>t.key===e)||null}function getSelectedFormalizadorCoverage(){const e=String(formalizadorState.coverageKey||"");return getFormalizadorCollaborators().find(t=>t.key===e)||null}function normalizeFormalizadorSearch(e){return normalizeQuickBetaValue(e)}function formalizadorIncludesQuery(e,t){const a=normalizeFormalizadorSearch(t).split(/\s+/).filter(Boolean);return a.length?a.every(o=>e.includes(o)):!0}function buildFormalizadorSearchText(...e){return normalizeFormalizadorSearch(e.map(t=>collectQuickBetaSearchText(t)).join(" "))}function getFormalizadorFilteredCollaborators(e="collaborator",t=18){const a=formalizadorState.queries?.[e]||"",o=getFormalizadorCollaborators(),n=e==="coverage"?formalizadorState.coverageKey:formalizadorState.collaboratorKey;return o.filter(s=>s.key!==n||e!=="coverage").filter(s=>{const i=buildFormalizadorSearchText(s.item,s.unit);return formalizadorIncludesQuery(i,a)}).sort((s,i)=>String(s.item?.nome||s.item?.colaborador||"").localeCompare(String(i.item?.nome||i.item?.colaborador||""),"pt-BR")).slice(0,t)}function getFormalizadorFilteredUnits(e=18){const t=formalizadorState.queries?.destination||"";return getFormalizadorUnits().filter(a=>formalizadorIncludesQuery(buildFormalizadorSearchText(a.unit),t)).sort((a,o)=>String(a.unit?.posto||a.unit?.nome||"").localeCompare(String(o.unit?.posto||o.unit?.nome||""),"pt-BR")).slice(0,e)}function formatFormalizadorType(e){return FORMALIZADOR_TYPES[e]?.label||String(e||"N\xE3o definido")}function formatFormalizadorStatus(e){return FORMALIZADOR_STATUS[e]||String(e||"Indefinido")}function formatFormalizadorPriority(e){return{baixa:"Baixa",normal:"Normal",alta:"Alta",urgente:"Urgente"}[e]||"Normal"}function formatFormalizadorDate(e){if(!e)return"N\xE3o informada";const t=normalizeFtDateKey(e);if(!t)return String(e);const[a,o,n]=t.split("-");return`${n}/${o}/${a}`}function getFormalizadorCollabName(e={}){return e.nome||e.colaborador||"Sem nome"}function getFormalizadorUnitName(e={}){return e.posto||e.nome||e.cliente||"Sem posto"}function getFormalizadorCompany(e={}){return pickFirstDefined(e?.empresa,e?.empresa_bombeiros,e?.empresa_servicos,e?.empresa_seguranca,e?.empresa_rb,e?.cliente)}function setFormalizadorStep(e){const t=parseInt(e,10);Number.isFinite(t)&&(formalizadorState.step=Math.min(Math.max(t,1),7),renderFormalizador())}function nextFormalizadorStep(){const e=validateFormalizadorStep(formalizadorState.step);if(!e.ok){showToast(e.message,"warning");return}setFormalizadorStep((formalizadorState.step||1)+1)}function prevFormalizadorStep(){setFormalizadorStep((formalizadorState.step||1)-1)}function selectFormalizadorType(e){FORMALIZADOR_TYPES[e]&&(formalizadorState.type=e,FORMALIZADOR_TYPES[e].needsDestination||(formalizadorState.destinationKey="",formalizadorState.queries.destination=""),renderFormalizador())}function setFormalizadorRequesterField(e,t){Object.prototype.hasOwnProperty.call(formalizadorState.requester,e)&&(formalizadorState.requester[e]=t,saveFormalizadorRequester())}function setFormalizadorFormField(e,t){Object.prototype.hasOwnProperty.call(formalizadorState.form,e)&&(formalizadorState.form[e]=t)}function setFormalizadorBenefit(e,t){Object.prototype.hasOwnProperty.call(formalizadorState.benefits,e)&&(formalizadorState.benefits[e]=t)}function setFormalizadorCoverageField(e,t){Object.prototype.hasOwnProperty.call(formalizadorState.coverage,e)&&(formalizadorState.coverage[e]=t,e==="tipo"&&t==="sem_cobertura"&&(formalizadorState.coverageKey="",formalizadorState.queries.coverage=""),renderFormalizador())}function setFormalizadorQuery(e,t){formalizadorState.queries||(formalizadorState.queries={}),formalizadorState.queries[e]=t||"",renderFormalizadorPicker(e)}function selectFormalizadorCollaborator(e){formalizadorState.collaboratorKey=String(e||""),renderFormalizador()}function selectFormalizadorDestination(e){formalizadorState.destinationKey=String(e||""),renderFormalizador()}function selectFormalizadorCoverage(e){formalizadorState.coverageKey=String(e||""),renderFormalizador()}function setFormalizadorHistoryFilter(e,t){formalizadorState.historyFilters||(formalizadorState.historyFilters={search:"",status:"all",tipo:"all"}),formalizadorState.historyFilters[e]=t||"",renderFormalizadorHistoryList()}function selectFormalizacaoHistory(e){formalizadorState.selectedHistoryId=String(e||""),renderFormalizadorHistoryList()}function renderFormalizador(){const e=document.getElementById("formalizador-root");if(!e)return;const t=(formalizadorState.history||[]).length,a=getFormalizadorCollaborators().length,o=getFormalizadorUnits().length,n=formalizadorCache.error?`
        <div class="formalizador-warning">
            N\xE3o consegui carregar o hist\xF3rico do Formalizador. Aplique o SQL <strong>formalizador_setup.sql</strong> no Supabase antes de formalizar.
        </div>
    `:"";e.innerHTML=`
        <div class="formalizador-hero">
            <div>
                ...
                <h2>Formalizador</h2>
                <p>Crie protocolos de troca de posto, remanejamento, desligamento, t\xE9rmino de experi\xEAncia, benef\xEDcios e cobertura sem alterar a planilha nem a base operacional.</p>
            </div>
            <div class="formalizador-hero-metrics">
                <span><strong>${a}</strong> colaboradores</span>
                <span><strong>${o}</strong> unidades</span>
                <span><strong>${t}</strong> protocolos</span>
            </div>
        </div>
        ${n}
    ...
    `}function renderFormalizadorStepper(){return`
        <div class="formalizador-stepper">
            ${[[1,"Solicitante"],[2,"Tipo"],[3,"Colaborador"],[4,"Mudan\xE7a"],[5,"Benef\xEDcios"],[6,"Cobertura"],[7,"Revis\xE3o"]].map(([t,a])=>`<button type="button" class="${t===formalizadorState.step?"active":t<formalizadorState.step?"done":""}" onclick="setFormalizadorStep(${t})"><span>${t}</span>${escapeHtml(a)}</button>`).join("")}
        </div>
    `}function renderFormalizadorCurrentStep(){switch(formalizadorState.step){case 1:return renderFormalizadorRequesterStep();case 2:return renderFormalizadorTypeStep();case 3:return renderFormalizadorCollaboratorStep();case 4:return renderFormalizadorChangeStep();case 5:return renderFormalizadorBenefitsStep();case 6:return renderFormalizadorCoverageStep();case 7:return renderFormalizadorReviewStep();default:return renderFormalizadorRequesterStep()}}function renderFormalizadorNav(){const e=formalizadorState.step===7;return`
        <div class="formalizador-nav">
            ...
            <div>
                ...
                ${e?'<button type="button" class="formalizador-btn primary" onclick="submitFormalizacao()">Formalizar</button>':'<button type="button" class="formalizador-btn primary" onclick="nextFormalizadorStep()">Avan\xE7ar</button>'}
            </div>
        </div>
    `}function renderFormalizadorRequesterStep(){const e=formalizadorState.requester||{};return`
        ...
        <div class="formalizador-form-grid">
            <label>
                Nome do solicitante *
                <input type="text" value="${escapeHtml(e.nome)}" oninput="setFormalizadorRequesterField('nome', this.value)" placeholder="Nome completo">
            </label>
            <label>
                Fun\xE7\xE3o / cargo *
                <input type="text" value="${escapeHtml(e.cargo)}" oninput="setFormalizadorRequesterField('cargo', this.value)" placeholder="Supervisor, coordenador...">
            </label>
            <label>
                Telefone
                <input type="tel" value="${escapeHtml(e.telefone)}" oninput="setFormalizadorRequesterField('telefone', this.value)" placeholder="(11) 99999-9999">
            </label>
            <label>
                E-mail
                <input type="email" value="${escapeHtml(e.email)}" oninput="setFormalizadorRequesterField('email', this.value)" placeholder="nome@empresa.com.br">
            </label>
        </div>
    `}function renderFormalizadorTypeStep(){return`
        <div class="formalizador-step-head">
            <span>Etapa 2</span>
            <h3>Tipo de formaliza\xE7\xE3o</h3>
            <p>Escolha o processo. Os campos obrigat\xF3rios mudam conforme o tipo.</p>
        </div>
        <div class="formalizador-type-grid">
            ${Object.entries(FORMALIZADOR_TYPES).map(([e,t])=>`
                <button type="button" class="formalizador-type-card ${formalizadorState.type===e?"selected":""}" onclick="selectFormalizadorType('${escapeHtml(e)}')">
                    <strong>${escapeHtml(t.label)}</strong>
                    <span>${escapeHtml(t.hint)}</span>
                </button>
            `).join("")}
        </div>
    `}function renderFormalizadorCollaboratorStep(){const e=getSelectedFormalizadorCollaborator();return`
        <div class="formalizador-step-head">
            <span>Etapa 3</span>
            <h3>Selecionar colaborador</h3>
            <p>Busque por nome, matr\xEDcula, RE, posto, cargo, empresa, cliente ou telefone.</p>
        </div>
        ${e?renderFormalizadorSelectedCollab(e,"Colaborador selecionado"):""}
    ...
    `}function renderFormalizadorChangeStep(){const e=getSelectedFormalizadorCollaborator(),t=getSelectedFormalizadorDestination(),a=FORMALIZADOR_TYPES[formalizadorState.type]?.needsDestination,o=e?.unit||null;return`
        <div class="formalizador-step-head">
            <span>Etapa 4</span>
            <h3>Mudan\xE7a solicitada</h3>
            <p>Defina data, prioridade, motivo e posto destino quando o processo exigir.</p>
        </div>
        <div class="formalizador-compare">
            <div>
                <span>Antes</span>
                ${e?renderFormalizadorSelectedCollab(e,"Situa\xE7\xE3o atual"):"<p>Selecione o colaborador na etapa anterior.</p>"}
                ${o?renderFormalizadorUnitSummary(o):""}
            </div>
            <div>
                <span>Depois</span>
                ${a?t?renderFormalizadorUnitSummary(t.unit):'<p class="formalizador-muted">Selecione o posto destino abaixo.</p>':'<p class="formalizador-muted">Este tipo n\xE3o exige posto destino.</p>'}
            </div>
        </div>
        <div class="formalizador-form-grid">
            <label>
            ...
            <label>
                ...
                <select onchange="setFormalizadorFormField('prioridade', this.value)">
                    ${["baixa","normal","alta","urgente"].map(n=>`<option value="${n}" ${formalizadorState.form.prioridade===n?"selected":""}>${formatFormalizadorPriority(n)}</option>`).join("")}
                </select>
            </label>
            <label>
                Motivo categoria
                <select onchange="setFormalizadorFormField('motivo_categoria', this.value)">
                    ${["","cobertura_contrato","ferias","falta","pedido_cliente","desempenho","experiencia","beneficios","outro"].map(n=>`<option value="${n}" ${formalizadorState.form.motivo_categoria===n?"selected":""}>${escapeHtml(n?n.replace(/_/g," "):"Selecione")}</option>`).join("")}
                </select>
            </label>
        </div>
        <label class="formalizador-wide-label">
            Observa\xE7\xF5es da mudan\xE7a
            <textarea rows="4" oninput="setFormalizadorFormField('motivo_observacao', this.value)" placeholder="Explique o motivo e qualquer condi\xE7\xE3o relevante.">${escapeHtml(formalizadorState.form.motivo_observacao||"")}</textarea>
        </label>
        ${a?`
            <div class="formalizador-search-box">
                ${ICONS.search}
        ...
        `:""}
    `}function renderFormalizadorBenefitsStep(){const e=formalizadorState.benefits||{};return`
        <div class="formalizador-step-head">
            <span>Etapa 5</span>
            <h3>Impactos em benef\xEDcios</h3>
            <p>Marque tudo que precisa ser avaliado pelo administrativo/DP.</p>
        </div>
        <div class="formalizador-check-grid">
            ${[["vale_transporte","Vale transporte","Marque se muda VT, rota, integra\xE7\xE3o ou desconto."],["vale_refeicao","Vale refei\xE7\xE3o / refei\xE7\xE3o no local","Marque se muda VR, refeit\xF3rio, marmita ou local de refei\xE7\xE3o."],["adicional_noturno","Adicional noturno","Marque se a mudan\xE7a altera per\xEDodo noturno."],["intrajornada","Intrajornada","Marque se altera pausa/intervalo contratual."],["escala_turno","Escala / turno","Marque se altera escala, hor\xE1rio ou turma."]].map(([a,o,n])=>`
                <label class="formalizador-check-card">
                    <input type="checkbox" ${e[a]?"checked":""} onchange="setFormalizadorBenefit('${a}', this.checked)">
                    <span>
        ...
        <label class="formalizador-wide-label">
            Observa\xE7\xF5es sobre benef\xEDcios
            <textarea rows="4" oninput="setFormalizadorBenefit('observacoes', this.value)" placeholder="Ex.: passa a ter adicional noturno; unidade destino possui refei\xE7\xE3o no local.">${escapeHtml(e.observacoes||"")}</textarea>
        </label>
    `}function renderFormalizadorCoverageStep(){const e=formalizadorState.coverage||{},t=getSelectedFormalizadorCoverage(),a=e.tipo!=="sem_cobertura";return`
        <div class="formalizador-step-head">
            <span>Etapa 6</span>
            <h3>Cobertura do posto</h3>
            <p>Informe se h\xE1 cobertura para o remanejamento ou se ficar\xE1 pendente.</p>
        </div>
        <div class="formalizador-radio-grid">
            ${[["sem_cobertura","Sem cobertura definida"],["cobertura_definida","Cobertura j\xE1 definida"],["temporaria","Cobertura tempor\xE1ria"],["definitiva","Cobertura definitiva"]].map(([o,n])=>`
                <label class="formalizador-radio-card ${e.tipo===o?"selected":""}">
                    <input type="radio" name="formalizador_coverage" value="${o}" ${e.tipo===o?"checked":""} onchange="setFormalizadorCoverageField('tipo', this.value)">
        ...
        <div class="formalizador-form-grid">
            <label>
                Per\xEDodo da cobertura
                <input type="text" value="${escapeHtml(e.periodo||"")}" oninput="setFormalizadorCoverageFieldNoRender('periodo', this.value)" placeholder="Ex.: 20/04 a 05/05, at\xE9 nova defini\xE7\xE3o">
            </label>
        </div>
        ${a?`
            ${t?renderFormalizadorSelectedCollab(t,"Substituto / cobertura selecionada"):""}
            <div class="formalizador-search-box">
                ${ICONS.search}
        ...
        `:`
            <div class="formalizador-warning compact">Sem cobertura definida: o protocolo ficar\xE1 com alerta de pend\xEAncia operacional.</div>
        `}
        <label class="formalizador-wide-label">
            Observa\xE7\xF5es de cobertura
            <textarea rows="4" oninput="setFormalizadorCoverageFieldNoRender('observacoes', this.value)" placeholder="Detalhe pend\xEAncias, substituto provis\xF3rio ou prazo.">${escapeHtml(e.observacoes||"")}</textarea>
        </label>
    `}function setFormalizadorCoverageFieldNoRender(e,t){Object.prototype.hasOwnProperty.call(formalizadorState.coverage,e)&&(formalizadorState.coverage[e]=t)}function renderFormalizadorReviewStep(){const e=validateFormalizador(),t=getSelectedFormalizadorCollaborator(),a=getSelectedFormalizadorDestination(),o=getFormalizadorDefaultRecipients();!formalizadorState.form.email_recipients&&o&&(formalizadorState.form.email_recipients=o);const n=buildFormalizadorPreview();return`
        <div class="formalizador-step-head">
            <span>Etapa 7</span>
            <h3>Revisar e formalizar</h3>
            <p>O supervisor revisa tudo antes de enviar e-mail ou WhatsApp. O site registra o protocolo, mas n\xE3o executa a mudan\xE7a.</p>
        </div>
        ${e.ok?"":`<div class="formalizador-warning">${escapeHtml(e.message)}</div>`}
        <div class="formalizador-review-grid">
            <section>
                ...
                <div class="formalizador-compare slim">
                    <div>
                        <span>Antes</span>
                        ${t?renderFormalizadorSelectedCollab(t,"Colaborador"):"<p>Sem colaborador.</p>"}
                    </div>
                    <div>
                        <span>Depois</span>
                        ${a?renderFormalizadorUnitSummary(a.unit):'<p class="formalizador-muted">Sem posto destino.</p>'}
                    </div>
                </div>
            </section>
            <section>
                <h4>Resumo</h4>
                ${renderFormalizadorSummaryList(n)}
            </section>
        </div>
        <label class="formalizador-wide-label">
            Destinat\xE1rios do e-mail
        ...
        <div class="formalizador-safe-note">O hist\xF3rico do Formalizador n\xE3o grava CPF, RG, PIS, CTPS nem endere\xE7o residencial.</div>
        ...
        `:""}
    `}function renderFormalizadorPicker(e){if(e==="destination"){const o=document.getElementById("formalizador-destination-results");o&&(o.innerHTML=renderFormalizadorUnitResults());return}const t=e==="coverage"?"formalizador-coverage-results":"formalizador-collaborator-results",a=document.getElementById(t);a&&(a.innerHTML=renderFormalizadorCollaboratorResults(e))}function renderFormalizadorCollaboratorResults(e="collaborator"){const t=getFormalizadorFilteredCollaborators(e,18);return t.length?t.map(a=>{const o=a.item||{},n=getDutyStatusByTurma(o.turma),s=n.code==="plantao"?"plantao":n.code==="folga"?"folga":"indefinido",i=escapeHtml(JSON.stringify(a.key));return`
            <button type="button" class="formalizador-result-card ${(e==="coverage"?formalizadorState.coverageKey:formalizadorState.collaboratorKey)===a.key?"selected":""}" onclick="${e==="coverage"?"selectFormalizadorCoverage":"selectFormalizadorCollaborator"}(${i})">
                <span class="formalizador-avatar">${escapeHtml(String(getFormalizadorCollabName(o)).slice(0,1)||"?")}</span>
                <span>
                    <strong>${escapeHtml(getFormalizadorCollabName(o))}</strong>
                    <small>${escapeHtml(o.matricula||o.re||"Sem matr\xEDcula")} \u2022 ${escapeHtml(o.cargo||"Sem cargo")}</small>
                    <small>${escapeHtml(o.posto||"Sem posto")} \u2022 ${escapeHtml(getFormalizadorCompany(o)||"")}</small>
                </span>
                <em class="${s}">${escapeHtml(n.text)}</em>
            </button>
        `}).join(""):'<div class="formalizador-empty">Nenhum colaborador encontrado.</div>'}function renderFormalizadorUnitResults(){const e=getFormalizadorFilteredUnits(18);return e.length?e.map(t=>{const a=t.unit||{},o=escapeHtml(JSON.stringify(t.key));return`
            <button type="button" class="formalizador-result-card unit ${formalizadorState.destinationKey===t.key?"selected":""}" onclick="selectFormalizadorDestination(${o})">
                <span class="formalizador-avatar unit">${escapeHtml(String(getFormalizadorUnitName(a)).slice(0,1)||"?")}</span>
                <span>
                    <strong>${escapeHtml(getFormalizadorUnitName(a))}</strong>
                    <small>${escapeHtml(a.cliente||getFormalizadorCompany(a)||"Sem cliente")} \u2022 ${escapeHtml(a.cidade||"")}</small>
                    <small>${escapeHtml(a.endereco_formatado||formatUnitAddress(a)||"Endere\xE7o n\xE3o informado")}</small>
                </span>
            </button>
        `}).join(""):'<div class="formalizador-empty">Nenhuma unidade encontrada.</div>'}function renderFormalizadorSelectedCollab(e,t="Colaborador"){const a=e?.item||{},o=getDutyStatusByTurma(a.turma);return`
        <article class="formalizador-selected-card ${o.code==="plantao"?"plantao":o.code==="folga"?"folga":"indefinido"}">
            <div>
                <span>${escapeHtml(t)}</span>
                <strong>${escapeHtml(getFormalizadorCollabName(a))}</strong>
                <small>${escapeHtml(a.matricula||a.re||"Sem matr\xEDcula")} \u2022 ${escapeHtml(a.cargo||"Sem cargo")}</small>
            </div>
            <div>
                <em>${escapeHtml(o.text)}</em>
                <small>${escapeHtml(a.posto||"Sem posto")}</small>
                <small>${escapeHtml(a.escala||"")} ${escapeHtml(a.turno||"")} Turma ${escapeHtml(a.turma||"-")}</small>
            </div>
        </article>
    `}function renderFormalizadorUnitSummary(e={}){return`
        <article class="formalizador-unit-summary">
            <strong>${escapeHtml(getFormalizadorUnitName(e))}</strong>
            <span>${escapeHtml(e.cliente||getFormalizadorCompany(e)||"Sem cliente")}</span>
            <small>${escapeHtml(e.unidade_de_negocio||"")}</small>
            <small>${escapeHtml(e.endereco_formatado||formatUnitAddress(e)||"Endere\xE7o n\xE3o informado")}</small>
            <small>${escapeHtml([e.email_supervisor_da_unidade,e.email_sesmt].filter(Boolean).join(" \u2022 "))}</small>
        </article>
    `}function renderFormalizadorSummaryList(e){return`<div class="formalizador-summary-list">${[["Tipo",formatFormalizadorType(formalizadorState.type)],["Prioridade",formatFormalizadorPriority(formalizadorState.form.prioridade)],["Data efetiva",formatFormalizadorDate(formalizadorState.form.data_efetiva)],["Data fim",formalizadorState.form.data_fim?formatFormalizadorDate(formalizadorState.form.data_fim):"N\xE3o se aplica"],["Cobertura",getFormalizadorCoverageLabel()],["Benef\xEDcios",getFormalizadorBenefitsLabel()],["Protocolo",e.protocolo||"Gerado ao formalizar"]].map(([a,o])=>`<div><span>${escapeHtml(a)}</span><strong>${escapeHtml(o||"N/I")}</strong></div>`).join("")}</div>`}function renderFormalizadorHistoryPanel(){const e=formalizadorState.historyFilters||{search:"",status:"all",tipo:"all"};return`
        ...
        <div class="formalizador-history-filters">
            <input type="text" value="${escapeHtml(e.search||"")}" oninput="setFormalizadorHistoryFilter('search', this.value)" placeholder="Protocolo, colaborador, posto...">
            <select onchange="setFormalizadorHistoryFilter('status', this.value)">
                <option value="all" ${e.status==="all"?"selected":""}>Todos os status</option>
                ${Object.entries(FORMALIZADOR_STATUS).map(([t,a])=>`<option value="${t}" ${e.status===t?"selected":""}>${escapeHtml(a)}</option>`).join("")}
            </select>
            <select onchange="setFormalizadorHistoryFilter('tipo', this.value)">
                <option value="all" ${e.tipo==="all"?"selected":""}>Todos os tipos</option>
                ${Object.entries(FORMALIZADOR_TYPES).map(([t,a])=>`<option value="${t}" ${e.tipo===t?"selected":""}>${escapeHtml(a.label)}</option>`).join("")}
            </select>
        </div>
        <div id="formalizador-history-list" class="formalizador-history-list">
            ${renderFormalizadorHistoryItems()}
        </div>
    `}function renderFormalizadorHistoryList(){const e=document.getElementById("formalizador-history-list");e&&(e.innerHTML=renderFormalizadorHistoryItems())}function getFilteredFormalizadorHistory(){const e=formalizadorState.historyFilters||{search:"",status:"all",tipo:"all"},t=buildFormalizadorSearchText(e.search||"");return(formalizadorState.history||[]).filter(a=>{if(e.status&&e.status!=="all"&&a.status!==e.status||e.tipo&&e.tipo!=="all"&&a.tipo!==e.tipo)return!1;if(!t)return!0;const o=buildFormalizadorSearchText(a);return formalizadorIncludesQuery(o,e.search)})}function renderFormalizadorHistoryItems(){const e=getFilteredFormalizadorHistory();if(!e.length)return'<div class="formalizador-empty">Nenhum protocolo encontrado.</div>';const t=e.find(a=>a.id===formalizadorState.selectedHistoryId)||e[0];return t?.id&&t.id!==formalizadorState.selectedHistoryId&&(formalizadorState.selectedHistoryId=t.id),`
        ${e.map(a=>`
            <button type="button" class="formalizador-history-card ${a.id===formalizadorState.selectedHistoryId?"selected":""}" onclick="selectFormalizacaoHistory('${escapeHtml(a.id)}')">
                <strong>${escapeHtml(a.protocolo||"Sem protocolo")}</strong>
                <span>${escapeHtml(formatFormalizadorType(a.tipo))}</span>
                <small>${escapeHtml(a.colaborador_nome||"Sem colaborador")} \u2022 ${escapeHtml(a.posto_atual||"-")} ${a.posto_destino?`\u2192 ${escapeHtml(a.posto_destino)}`:""}</small>
                <em class="${escapeHtml(a.status||"")}">${escapeHtml(formatFormalizadorStatus(a.status))}</em>
            </button>
        `).join("")}
        ${t?renderFormalizadorHistoryDetail(t):""}
    `}function renderFormalizadorHistoryDetail(e){const t=formalizadorState.events?.[String(e.id)]||[];return`
        <div class="formalizador-history-detail">
            <div class="formalizador-history-detail-head">
                <div>
                    <span>Detalhe</span>
                    <strong>${escapeHtml(e.protocolo||"Sem protocolo")}</strong>
                </div>
                <select onchange="updateFormalizacaoStatus('${escapeHtml(e.id)}', this.value)">
                    ${Object.entries(FORMALIZADOR_STATUS).map(([a,o])=>`<option value="${a}" ${e.status===a?"selected":""}>${escapeHtml(o)}</option>`).join("")}
                </select>
            </div>
            ${renderFormalizadorSummaryListFromRecord(e)}
            <div class="formalizador-post-actions compact">
                <button type="button" onclick="copyFormalizacaoText('${escapeHtml(e.id)}')">Copiar</button>
                <button type="button" onclick="openFormalizacaoEmail('${escapeHtml(e.id)}')">E-mail</button>
                <button type="button" onclick="shareFormalizacaoWhatsapp('${escapeHtml(e.id)}')">WhatsApp</button>
            </div>
            <div class="formalizador-timeline">
                <strong>Linha do tempo</strong>
                ${(t.length?t:[{status_novo:e.status,ator_nome:e.solicitante_nome,created_at:e.created_at,observacao:"Formaliza\xE7\xE3o registrada."}]).map(a=>`
                    <div>
                        <span>${escapeHtml(formatFormalizadorDateTime(a.created_at))}</span>
                        <p>${escapeHtml(formatFormalizadorStatus(a.status_novo))} por ${escapeHtml(a.ator_nome||"Sistema")}</p>
                        ${a.observacao?`<small>${escapeHtml(a.observacao)}</small>`:""}
                    </div>
                `).join("")}
            </div>
        </div>
    `}function renderFormalizadorSummaryListFromRecord(e){return`<div class="formalizador-summary-list">${[["Status",formatFormalizadorStatus(e.status)],["Tipo",formatFormalizadorType(e.tipo)],["Solicitante",`${e.solicitante_nome||""} ${e.solicitante_cargo?`(${e.solicitante_cargo})`:""}`],["Colaborador",`${e.colaborador_nome||""} ${e.colaborador_matricula?`- ${e.colaborador_matricula}`:""}`],["Atual",e.posto_atual||"-"],["Destino",e.posto_destino||"-"],["Data efetiva",formatFormalizadorDate(e.data_efetiva)],["Cobertura",getFormalizadorCoverageLabel(e.cobertura_json)],["Criado em",formatFormalizadorDateTime(e.created_at)]].map(([a,o])=>`<div><span>${escapeHtml(a)}</span><strong>${escapeHtml(o||"N/I")}</strong></div>`).join("")}</div>`}function formatFormalizadorDateTime(e){if(!e)return"-";const t=new Date(e);return Number.isNaN(t.getTime())?String(e):t.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"})}function validateFormalizadorStep(e){const t=formalizadorState.requester||{};if(e>=1&&(!String(t.nome||"").trim()||!String(t.cargo||"").trim()))return{ok:!1,message:"Informe nome e cargo do solicitante."};if(e>=2&&!formalizadorState.type)return{ok:!1,message:"Escolha o tipo de formaliza\xE7\xE3o."};if(e>=3&&!getSelectedFormalizadorCollaborator())return{ok:!1,message:"Selecione o colaborador."};if(e>=4){if(!formalizadorState.form.data_efetiva)return{ok:!1,message:"Informe a data efetiva."};if(FORMALIZADOR_TYPES[formalizadorState.type]?.needsDestination&&!getSelectedFormalizadorDestination())return{ok:!1,message:"Selecione o posto destino."};if(formalizadorState.type==="termino_experiencia"&&!formalizadorState.form.data_fim)return{ok:!1,message:"Informe a data de t\xE9rmino da experi\xEAncia."}}return{ok:!0,message:""}}function validateFormalizador(){return validateFormalizadorStep(7)}function getFormalizadorBenefitsLabel(e=formalizadorState.benefits){const t=[];return e?.vale_transporte&&t.push("VT"),e?.vale_refeicao&&t.push("VR/refei\xE7\xE3o"),e?.adicional_noturno&&t.push("Adicional noturno"),e?.intrajornada&&t.push("Intrajornada"),e?.escala_turno&&t.push("Escala/turno"),t.length?t.join(", "):"Sem impacto informado"}function getFormalizadorCoverageLabel(e=formalizadorState.coverage){const a={sem_cobertura:"Sem cobertura definida",cobertura_definida:"Cobertura j\xE1 definida",temporaria:"Cobertura tempor\xE1ria",definitiva:"Cobertura definitiva"}[e?.tipo]||"N\xE3o informado",o=getSelectedFormalizadorCoverage(),n=e?.substituto?.nome||(o?.item?getFormalizadorCollabName(o.item):"");return[a,n].filter(Boolean).join(" - ")}function getFormalizadorDefaultRecipients(){const e=getSelectedFormalizadorDestination()?.unit||null,a=getSelectedFormalizadorCollaborator()?.unit||null,o=e||a||{},n=[o.email_supervisor_da_unidade,o.email_sesmt,o.email_dp,o.email_rh,o.email].map(s=>String(s||"").trim()).filter(Boolean);return Array.from(new Set(n)).join(", ")}function generateFormalizadorProtocol(){const e=getTodayKey().replace(/-/g,""),t=Math.random().toString(36).slice(2,7).toUpperCase();return`FP-${e}-${t}`}function getSafeFormalizadorCollaboratorSnapshot(e){const t=e?.item||{};return{matricula:t.matricula||"",re:t.re||"",re_padrao:t.re_padrao||"",re_novo:t.re_novo||t.re_folha||"",nome:getFormalizadorCollabName(t),cargo:t.cargo||"",posto:t.posto||"",escala:t.escala||"",turno:t.turno||"",empresa:getFormalizadorCompany(t)||"",cliente:t.cliente||"",unidade_de_negocio:t.unidade_de_negocio||"",turma:t.turma||"",telefone:t.telefone||""}}function getSafeFormalizadorUnitSnapshot(e={}){return{posto:getFormalizadorUnitName(e),cliente:e.cliente||"",empresa:getFormalizadorCompany(e)||"",unidade_de_negocio:e.unidade_de_negocio||"",endereco_formatado:e.endereco_formatado||formatUnitAddress(e)||"",cidade:e.cidade||"",estado:e.estado||"",email_supervisor_da_unidade:e.email_supervisor_da_unidade||"",email_sesmt:e.email_sesmt||"",refeicao_no_local:pickFirstDefined(e.refeicao_no_local,e.refeicao,e.vale_refeicao),intrajornada:e.intrajornada||""}}function buildFormalizadorPreview(e=""){const t=buildFormalizadorPayload(e||"PREVIEW",{preview:!0});return{protocolo:e||"",subject:t.email_subject,body:t.email_body,whatsapp:t.whatsapp_text}}function buildFormalizadorPayload(e=generateFormalizadorProtocol(),t={}){const a=getSelectedFormalizadorCollaborator(),o=getSelectedFormalizadorDestination(),n=getSelectedFormalizadorCoverage(),s=a?.item||{},i=o?.unit||null,r=a?.unit||null,l={...formalizadorState.coverage||{},substituto:n?getSafeFormalizadorCollaboratorSnapshot(n):null},c={colaborador:a?getSafeFormalizadorCollaboratorSnapshot(a):null,unidade_atual:r?getSafeFormalizadorUnitSnapshot(r):null,unidade_destino:i?getSafeFormalizadorUnitSnapshot(i):null,cobertura:l.substituto,email_recipients:formalizadorState.form.email_recipients||getFormalizadorDefaultRecipients(),gerado_em:new Date().toISOString()},d={protocolo:e,tipo:formalizadorState.type||"",status:"formalizado",prioridade:formalizadorState.form.prioridade||"normal",solicitante_nome:String(formalizadorState.requester.nome||"").trim(),solicitante_cargo:String(formalizadorState.requester.cargo||"").trim(),solicitante_telefone:String(formalizadorState.requester.telefone||"").trim(),solicitante_email:String(formalizadorState.requester.email||"").trim(),colaborador_matricula:s.matricula||"",colaborador_re:s.re||s.re_padrao||s.re_novo||"",colaborador_nome:getFormalizadorCollabName(s),colaborador_cargo:s.cargo||"",posto_atual:s.posto||"",posto_destino:i?getFormalizadorUnitName(i):"",data_efetiva:normalizeFtDateKey(formalizadorState.form.data_efetiva)||null,data_fim:normalizeFtDateKey(formalizadorState.form.data_fim)||null,motivo_categoria:formalizadorState.form.motivo_categoria||"",motivo_observacao:formalizadorState.form.motivo_observacao||"",beneficios_json:{...formalizadorState.benefits||{}},cobertura_json:l,snapshot_json:c};return d.email_subject=buildFormalizadorEmailSubject(d,t.preview),d.email_body=buildFormalizadorMessageBody(d),d.whatsapp_text=buildFormalizadorWhatsappText(d),d}function buildFormalizadorEmailSubject(e,t=!1){const a=formatFormalizadorType(e.tipo).toUpperCase(),o=e.colaborador_nome||"COLABORADOR",n=e.posto_atual||"-",s=e.posto_destino||"-",i=t?"PR\xC9VIA":e.protocolo;return`[FORMALIZA\xC7\xC3O] ${a} - ${o} - ${n} -> ${s} - ${i}`}function buildFormalizadorMessageBody(e){const t=getFormalizadorBenefitsLabel(e.beneficios_json),a=getFormalizadorCoverageLabel(e.cobertura_json);return[`Protocolo: ${e.protocolo}`,`Tipo: ${formatFormalizadorType(e.tipo)}`,`Status inicial: ${formatFormalizadorStatus(e.status)}`,`Prioridade: ${formatFormalizadorPriority(e.prioridade)}`,"","Solicitante",`Nome: ${e.solicitante_nome}`,`Fun\xE7\xE3o/Cargo: ${e.solicitante_cargo}`,`Telefone: ${e.solicitante_telefone||"-"}`,`E-mail: ${e.solicitante_email||"-"}`,"","Colaborador",`Nome: ${e.colaborador_nome}`,`Matr\xEDcula: ${e.colaborador_matricula||"-"}`,`RE: ${e.colaborador_re||"-"}`,`Cargo: ${e.colaborador_cargo||"-"}`,"","Situa\xE7\xE3o atual",`Posto atual: ${e.posto_atual||"-"}`,"","Altera\xE7\xE3o solicitada",`Posto destino: ${e.posto_destino||"-"}`,`Data efetiva: ${formatFormalizadorDate(e.data_efetiva)}`,`Data fim: ${e.data_fim?formatFormalizadorDate(e.data_fim):"-"}`,`Motivo/categoria: ${e.motivo_categoria||"-"}`,`Observa\xE7\xF5es: ${e.motivo_observacao||"-"}`,"","Impactos em benef\xEDcios",t,e.beneficios_json?.observacoes?`Observa\xE7\xF5es: ${e.beneficios_json.observacoes}`:"","","Cobertura",a,e.cobertura_json?.periodo?`Per\xEDodo: ${e.cobertura_json.periodo}`:"",e.cobertura_json?.observacoes?`Observa\xE7\xF5es: ${e.cobertura_json.observacoes}`:"","","Confirma\xE7\xE3o","Esta mensagem formaliza a solicita\xE7\xE3o registrada no site Dunamis Pro. A execu\xE7\xE3o administrativa da mudan\xE7a deve ser validada pelos respons\xE1veis antes de qualquer altera\xE7\xE3o operacional."].filter(n=>n!=="").join(`
`)}function buildFormalizadorWhatsappText(e){return[`Formaliza\xE7\xE3o ${e.protocolo}`,`${formatFormalizadorType(e.tipo)} | ${formatFormalizadorStatus(e.status)}`,`Solicitante: ${e.solicitante_nome} (${e.solicitante_cargo})`,`Colaborador: ${e.colaborador_nome} - ${e.colaborador_matricula||e.colaborador_re||"-"}`,`Atual: ${e.posto_atual||"-"}`,`Destino: ${e.posto_destino||"-"}`,`Data efetiva: ${formatFormalizadorDate(e.data_efetiva)}`,`Benef\xEDcios: ${getFormalizadorBenefitsLabel(e.beneficios_json)}`,`Cobertura: ${getFormalizadorCoverageLabel(e.cobertura_json)}`,e.motivo_observacao?`Obs.: ${e.motivo_observacao}`:""].filter(Boolean).join(`
`)}async function submitFormalizacao(){const e=validateFormalizador();if(!e.ok){showToast(e.message,"warning");return}if(!supabaseClient){showToast("Supabase n\xE3o configurado. N\xE3o foi poss\xEDvel salvar o protocolo.","error");return}const t=generateFormalizadorProtocol(),a=buildFormalizadorPayload(t);try{const{data:o,error:n}=await supabaseClient.from(SUPABASE_TABLES.formalizacoes_postos).insert(a).select("*").single();if(n)throw n;const s=o||a;if(s.id){const{error:i}=await supabaseClient.from(SUPABASE_TABLES.formalizacoes_status_events).insert({formalizacao_id:s.id,status_anterior:null,status_novo:"formalizado",ator_nome:a.solicitante_nome||"Solicitante",observacao:"Formaliza\xE7\xE3o criada pelo site."});i&&AppErrorHandler.capture(i,{scope:"formalizador-submit-event"},{silent:!0})}formalizadorState.lastCreatedId=s.id||"",formalizadorState.selectedHistoryId=s.id||formalizadorState.selectedHistoryId,formalizadorCache.updatedAt=0,await fetchFormalizacoes(!0),copyTextToClipboard(s.email_body||a.email_body),showToast(`Protocolo ${s.protocolo||t} formalizado e texto copiado.`,"success"),renderFormalizador()}catch(o){AppErrorHandler.capture(o,{scope:"formalizador-submit"},{silent:!0}),showToast(`Erro ao salvar formaliza\xE7\xE3o: ${o?.message||o}`,"error")}}function getFormalizacaoRecord(e=""){const t=String(e||formalizadorState.lastCreatedId||formalizadorState.selectedHistoryId||"");return t&&(formalizadorState.history||[]).find(a=>String(a.id)===t||String(a.protocolo)===t)||null}function copyFormalizacaoText(e=""){const t=getFormalizacaoRecord(e);if(t?.email_body){copyTextToClipboard(t.email_body);return}const a=buildFormalizadorPreview();copyTextToClipboard(a.body)}function openFormalizacaoEmail(e=""){const t=getFormalizacaoRecord(e),a=t||buildFormalizadorPayload(generateFormalizadorProtocol(),{preview:!0}),o=t?getFormalizadorRecipientsFromRecord(t):formalizadorState.form.email_recipients||getFormalizadorDefaultRecipients(),n=a.email_subject||a.subject||"",s=a.email_body||a.body||"";copyTextToClipboard(s);const i=`https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(o||"")}&su=${encodeURIComponent(n)}&body=${encodeURIComponent(s)}`;if(!window.open(i,"_blank","noopener,noreferrer")){const l=new URLSearchParams;n&&l.set("subject",n),s&&l.set("body",s),window.location.href=`mailto:${o||""}?${l.toString()}`}}async function shareFormalizacaoWhatsapp(e=""){const a=getFormalizacaoRecord(e)?.whatsapp_text||buildFormalizadorPreview().whatsapp;copyTextToClipboard(a);try{if(navigator.share){await navigator.share({text:a});return}}catch{}openWhatsApp("",a)}function getFormalizadorRecipientsFromRecord(e={}){if(e.email_recipients)return e.email_recipients;const t=e.snapshot_json||{};if(t.email_recipients)return t.email_recipients;const a=t.unidade_destino||t.unidade_atual||{};return[a.email_supervisor_da_unidade,a.email_sesmt].filter(Boolean).join(", ")}async function updateFormalizacaoStatus(e,t){const a=getFormalizacaoRecord(e);if(!a||!FORMALIZADOR_STATUS[t]||a.status===t)return;if(!supabaseClient){showToast("Supabase n\xE3o configurado.","error");return}const o=window.prompt("Observa\xE7\xE3o da mudan\xE7a de status (opcional):","");if(o===null){renderFormalizador();return}const n=o||"";try{const{error:s}=await supabaseClient.from(SUPABASE_TABLES.formalizacoes_status_events).insert({formalizacao_id:a.id,status_anterior:a.status||null,status_novo:t,ator_nome:formalizadorState.requester?.nome||"Operador",observacao:n});if(s)throw s;const{error:i}=await supabaseClient.from(SUPABASE_TABLES.formalizacoes_postos).update({status:t}).eq("id",a.id);if(i)throw i;await fetchFormalizacoes(!0),showToast("Status atualizado.","success"),renderFormalizador()}catch(s){AppErrorHandler.capture(s,{scope:"formalizador-status-update"},{silent:!0}),showToast(`Erro ao atualizar status: ${s?.message||s}`,"error"),renderFormalizador()}}function getFtTodayByUnit(e,t){const a=getTodayKey(),o=normalizeUnitKey(e);return getFtOperationalItems(ftLaunches).filter(n=>{if(!isFtActive(n)||!n.date||n.date!==a||t&&t!=="all"&&n.group&&n.group!==t)return!1;const s=normalizeUnitKey(n.unitTarget||n.unitCurrent||"");return s&&s===o})}function renderizarUnidades(){const e=document.getElementById("units-list"),t=document.getElementById("unit-search-input")?.value.toUpperCase()||"",a=document.getElementById("unit-group-filter")?.value||"all",o=document.getElementById("unit-status-filter")?.value||"all",n=document.getElementById("unit-label-filter")?.value||"all",s=normalizeDateRange(unitDateFilter.from,unitDateFilter.to);atualizarEstatisticas(currentData,a);let i=currentData;a!=="all"&&(i=i.filter(d=>d.grupo===a));const r={};i.forEach(d=>{r[d.posto]||(r[d.posto]=[]),r[d.posto].push(d)});let l=Object.keys(r).sort();t&&(l=l.filter(d=>d.includes(t)));const c=l.map(d=>{const m=r[d];m.sort((F,T)=>F.nome.localeCompare(T.nome));const g=m.filter(F=>{const T=getStatusInfo(F),B=T.text.includes("PLANT\xC3O")||T.text.includes("FT");return(o==="all"||o==="plantao")&&B}),p=m.filter(F=>{const B=getStatusInfo(F).text==="FOLGA";return(o==="all"||o==="folga")&&B}),f=F=>n==="all"?F:n==="none"?F.filter(T=>!T.rotulo):F.filter(T=>(T.rotulo||"").split(",").includes(n)),u=f(g),b=f(p);if(u.length===0&&b.length===0)return"";const v="unit-"+d.replace(/[^a-zA-Z0-9]/g,"-"),y=hiddenUnits.has(d),k=minimizedUnits.has(d),E=unitMetadata[d]||{};let $="";E.rotulo&&($=E.rotulo.split(",").map(T=>{const B={REFEICAO:"Refei\xE7\xE3o no Local",VT:"Vale Transporte"};return`<span class="unit-label-badge">${T==="OUTRO"&&E.detalhe?E.detalhe:B[T]||T}</span>`}).join(""));const S=!!E.rotulo,_=getFtTodayByUnit(d,a==="all"?"":a),R=_.length?`<span class="unit-ft-badge">FT hoje: ${_.length}</span>`:"",D=buildFtWeekPreviewHtmlForUnit(d,a==="all"?"":a),N=hasDateRangeFilter(s)?getFtItemsForUnitInRange(d,s.from,s.to,a==="all"?"":a):[];if(hasDateRangeFilter(s)&&!N.length)return"";const A=F=>hasDateRangeFilter(s)?F.filter(T=>matchesFtDateFilterForCollaborator(T.re,s)):F,C=A(u),I=A(b);if(C.length===0&&I.length===0)return"";const x=JSON.stringify(d),h=escapeHtml(x),w=escapeHtml(d);return`
...
`)}`:`Reciclagem: ${r.join(", ")||"sem dados"}`}}async function loadReciclagemData(e=!1){isDashboardFeatureEnabled("reciclagem")&&(!e&&reciclagemLoadedAt||(setAppState("reciclagemData",{},{silent:!0}),setAppState("reciclagemLoadedAt",new Date,{silent:!0}),e&&showToast("Reciclagem via Supabase ser\xE1 ativada em breve.","info")))}async function refreshReciclagemIfNeeded(){if(!isDashboardFeatureEnabled("reciclagem"))return;const e=reciclagemLoadedAt?reciclagemLoadedAt.getTime():0;await loadReciclagemData(!1),(reciclagemLoadedAt?reciclagemLoadedAt.getTime():0)>e&&currentTab==="reciclagem"&&renderReciclagem()}function startReciclagemAutoRefresh(){}function getReciclagemSheetLabel(e){return{ASO:"ASO",REQUALIFICACAO:"RECICLAGEM",NR10:"NR 10",NR20:"NR 20",NR33:"NR 33",NR35:"NR 35",DEA:"DEA",HELIPONTO:"HELIPONTO",UNIFORME:"UNIFORME",PCMSO:"PCMSO",PGR:"PGR"}[e]||e}function buildReciclagemMessage(e,t){const a=reciclagemTemplates.find(n=>n.id===e);if(!a)return"";const o=t?.expiry?formatDateOnly(t.expiry):t?.expiryIso||"";return a.text.replace(/{NOME}/g,t?.name||"").replace(/{RE}/g,t?.re||"").replace(/{TIPO}/g,t?.typeLabel||"").replace(/{VENCIMENTO}/g,o||"")}function openReciclagemWhatsapp(e,t){const a=buildReciclagemMessage(e,t);if(!a){showToast("Selecione um modelo de mensagem.","error");return}const o=t?.phone||"";openWhatsApp(o,a)}function copyReciclagemMessage(e,t){const a=buildReciclagemMessage(e,t);if(!a){showToast("Selecione um modelo de mensagem.","error");return}copyTextToClipboard(a)}function getReciclagemSheetKeysForTab(e){const t=CONFIG?.reciclagem?.sheets||{},a=Object.keys(t);return e==="unit"?a.filter(o=>t[o].match==="unit"):a.filter(o=>t[o].match!=="unit")}function applyReciclagemFilters(e,t){let a=e;if(t){const o=t.toUpperCase();a=a.filter(n=>(n.name||"").toUpperCase().includes(o)||(n.re||"").toUpperCase().includes(o)||(n.unit||"").toUpperCase().includes(o))}return a}function switchReciclagemTab(e){reciclagemTab=e,document.querySelectorAll(".reciclagem-tab").forEach(t=>t.classList.remove("active")),document.querySelector(`.reciclagem-tab[onclick="switchReciclagemTab('${e}')"]`)?.classList.add("active"),renderReciclagem()}function toggleReciclagemOnlyExpired(){reciclagemOnlyExpired=!reciclagemOnlyExpired;const e=document.getElementById("reciclagem-only-expired");e&&e.classList.toggle("active",reciclagemOnlyExpired),renderReciclagem()}function setReciclagemStatusFilter(e){const t=document.getElementById("reciclagem-status-filter");t&&(t.value=e,renderReciclagem())}function focusReciclagemUnit(e){const t=document.getElementById("reciclagem-search");t&&(t.value=e||"",renderReciclagem())}function renderReciclagemSkeleton(e,t=5){e.innerHTML=Array.from({length:t}).map(()=>`
    ...
    `,t.classList.remove("hidden")}async function renderReciclagem(){if(!isDashboardFeatureEnabled("reciclagem"))return;const e=document.getElementById("reciclagem-list");if(!e)return;const t=document.getElementById("reciclagem-type-counts");t&&(t.innerHTML=""),renderReciclagemSkeleton(e,6),bindReciclagemActions(e);try{await loadReciclagemData(!1)}catch(p){AppErrorHandler.capture(p,{scope:"render-reciclagem-load"},{silent:!0}),e.innerHTML='<div class="empty-state">Erro ao carregar reciclagem.</div>';return}let a,o,n,s;try{a=document.getElementById("reciclagem-sheet-filter"),o=document.getElementById("reciclagem-status-filter"),n=document.getElementById("reciclagem-search"),s=getReciclagemSheetKeysForTab(reciclagemTab)}catch(p){AppErrorHandler.capture(p,{scope:"render-reciclagem-filters"},{silent:!0}),e.innerHTML='<div class="empty-state">Erro ao carregar reciclagem.</div>';return}if(a&&!a.dataset.ready&&(a.addEventListener("change",renderReciclagem),a.dataset.ready="1"),o&&!o.dataset.ready&&(o.addEventListener("change",renderReciclagem),o.dataset.ready="1"),n&&!n.dataset.ready&&(n.addEventListener("input",()=>{clearTimeout(n._timer),n._timer=setTimeout(renderReciclagem,200)}),n.dataset.ready="1"),a){const p=a.value||"all";a.innerHTML='<option value="all">Todas as reciclagens</option>'+s.map(f=>`<option value="${f}">${getReciclagemSheetLabel(f)}</option>`).join(""),a.value=s.includes(p)?p:"all"}const i=a?.value||"all",r=o?.value||"all",l=n?.value.trim()||"",c=document.getElementById("reciclagem-only-expired");c&&c.classList.toggle("active",reciclagemOnlyExpired),document.querySelectorAll(".reciclagem-quick .filter-chip").forEach(p=>{const f=p.dataset.status===r;p.classList.toggle("active",f)});const d=[];if(reciclagemTab==="unit"){const p=[...new Set(currentData.map(u=>u.posto).filter(Boolean))].sort(),f=i==="all"?s:[i];p.forEach(u=>{const b=[];f.forEach(v=>{const y=reciclagemData[v]?.entries||[],k=findReciclagemUnitEntry(y,u),E=getReciclagemOverride(v,normalizeUnitKey(u)),$=E?.expiry?parseDateFlexible(E.expiry):k?.expiry||null;if(!$&&!k){b.push({key:v,label:getReciclagemSheetLabel(v),status:"unknown",expiry:null,dateLabel:"Sem dados"});return}if(!$){b.push({key:v,label:getReciclagemSheetLabel(v),status:"unknown",expiry:null,dateLabel:"sem data"});return}const S=calcReciclagemStatus($);b.push({key:v,label:getReciclagemSheetLabel(v),status:S.status,expiry:$,dateLabel:formatDateOnly($)})}),d.push({type:"ALL",typeLabel:"Documentos",unit:u,status:"mixed",detailItems:b,collabStatus:getCollabStatusFromDetails(b),match:"unit"})})}else{const p=i==="all"?s:[i];getUniqueReciclagemCollabs(currentData).forEach(u=>{let b={ok:0,due:0,expired:0,unknown:0},v=!1;const y=[];if(p.forEach($=>{const S=reciclagemData[$]?.entries||[],_=findReciclagemEntry(S,u.re,u.nome),R=getReciclagemOverride($,u.re),D=_?.expiry||(R?.expiry?parseDateFlexible(R.expiry):null);if(!D&&!_){y.push({key:$,label:getReciclagemSheetLabel($),status:"unknown",expiry:null,dateLabel:"Sem dados"});return}if(v=!0,!D){b.unknown+=1,y.push({key:$,label:getReciclagemSheetLabel($),status:"unknown",expiry:null,dateLabel:"sem data"});return}const N=calcReciclagemStatus(D);b[N.status]=(b[N.status]||0)+1,y.push({key:$,label:getReciclagemSheetLabel($),status:N.status,expiry:D,dateLabel:formatDateOnly(D)})}),!v){d.push({type:"ALL",typeLabel:"Reciclagens",name:u.nome,re:u.re,phone:getPhoneForCollab(u.re)||"",unit:u.posto||"",status:"unknown",days:null,expiry:null,expiryIso:"",summary:"Sem dados",detailItems:y,collabStatus:"unknown",match:"re"});return}const k=[],E=[];b.expired&&k.push(`${b.expired} vencida(s)`),b.due&&k.push(`${b.due} pr\xF3xima(s)`),b.ok&&k.push(`${b.ok} em dia`),!k.length&&b.unknown&&k.push("sem data"),y.forEach($=>{const S=$.status==="expired"?"vencido":$.status==="due"?"pr\xF3ximo":$.status==="ok"?"em dia":"sem data";E.push(`${$.label}: ${S} (${$.dateLabel})`)}),d.push({type:"ALL",typeLabel:"Reciclagens",name:u.nome,re:u.re,phone:getPhoneForCollab(u.re)||"",unit:u.posto||"",status:"mixed",days:null,expiry:null,expiryIso:"",summary:k.join(", "),summaryDetail:E.join(" | "),detailItems:y,collabStatus:getCollabStatusFromDetails(y),match:"re"})})}const g=applyReciclagemFilters(d,l).filter(p=>reciclagemTab!=="colab"?!(reciclagemOnlyExpired&&p.status!=="expired"||r&&r!=="all"&&p.status!==r):!p.detailItems||!p.detailItems.length?!reciclagemOnlyExpired&&(r==="all"||r==="unknown"):reciclagemOnlyExpired?p.collabStatus==="expired":r&&r!=="all"?p.collabStatus===r:!0);if(t){const p={};reciclagemTab==="colab"?g.forEach(u=>{(u.detailItems||[]).forEach(b=>{p[b.label]||(p[b.label]={expired:0,due:0,ok:0,unknown:0}),p[b.label][b.status]=(p[b.label][b.status]||0)+1})}):g.forEach(u=>{const b=u.typeLabel||u.type;p[b]||(p[b]={expired:0,due:0,ok:0,unknown:0}),p[b][u.status]=(p[b][u.status]||0)+1});const f=Object.keys(p).map(u=>{const b=p[u];return`<div class="reciclagem-type-chip">
               ...
               </div>`:"",S=p.match==="unit"&&v&&p.type!=="ALL",_=i!=="all"?(p.detailItems||[]).filter(A=>A.key===i):p.detailItems||[],R=p.match==="re"&&_.some(A=>A.status==="expired")?'<span class="reciclagem-badge" title="Vencido"></span>':"",D=p.match==="re"?_.map(A=>{const C=A.status==="expired"?"Vencido":A.status==="due"?"Pr\xF3ximo":A.status==="ok"?"Em dia":"Sem data";return`
        ...
        <div class="result-card ${F} ${U?"card-homenageado":""}" data-collab-re="${e.re}" style="border-left: 5px solid ${o.color}">
            <div class="card-header">
                ...
                <div class="header-right card-actions-bar">
                    ...
                    <button class="edit-btn-icon map-icon ${D}" onclick="openAddressModalForCollaborator(${u}, ${b}, ${y})" title="${N}" ${R?"":"disabled"}>${ICONS.mapPin}</button>
    ...
    `;const m=document.getElementById("troca-history-search");m&&(m.value=trocaHistoryFilter.search||"",m.addEventListener("input",u=>{trocaHistoryFilter.search=u.target.value||"",renderLancamentosTrocaHistorico()}));const g=document.getElementById("troca-history-unit");g&&(g.value=trocaHistoryFilter.unit||"",g.addEventListener("change",u=>{trocaHistoryFilter.unit=u.target.value||"",renderLancamentosTrocaHistorico()}));const p=document.getElementById("troca-history-status");p&&(p.value=trocaHistoryFilter.status||"all",p.addEventListener("change",u=>{trocaHistoryFilter.status=u.target.value||"all",renderLancamentosTrocaHistorico()}));const f=document.getElementById("troca-history-sort");f&&(f.value=trocaHistoryFilter.sort||"date_desc",f.addEventListener("change",u=>{trocaHistoryFilter.sort=u.target.value||"date_desc",renderLancamentosTrocaHistorico()}))}function renderLancamentosDashboard(){const e=document.getElementById("lancamentos-panel-dashboard");if(!e)return;const t=applyFtFilters(ftLaunches),a=buildFtDashboardStats(t),o=getMonthlyGidChecklistStatus(new Date),n=[];o.ftOk||n.push("FT");const s=n.length?`<div class="lancamentos-gid-alert">Lembrete ${o.monthLabel}: faltando cadastrar GID mensal de FT (config.js).</div>`:"",{pending:i,submitted:r,launched:l,total:c,avgPerDay:d,launchRate:m,topUnits:g,topPeople:p,topReasons:f,topShifts:u,topTimes:b,topGroups:v,topPendingUnits:y,weekdayEntries:k,recentPending:E,recentLaunched:$,missingUnit:S,missingRe:_,missingDate:R}=a;e.innerHTML=`
        ...
        <div class="lancamentos-report-grid">
            ...
            <div class="report-card">
                ...
                <div class="report-list">${buildReportRows([{label:"Sem unidade",value:S},{label:"Sem RE",value:_},{label:"Sem data",value:R}].filter(D=>D.value>0))}</div>
            </div>
        </div>
